...
<body>
	<div class="pre-loader">
		<div class="pre-loader-box">
			<div class="loader-logo"><img src="<?= base_url() ?>deskapp/vendors/images/deskapp-logo.svg" alt=""></div>
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
	...
	<script>
		...
		function get_vehicle(el) {
			if ($('#dd_vehicle option:selected').html() == 'Taksi Online') {
				$('#drop_plat').attr('disabled','disabled')
			} else {
				$('#drop_plat').removeAttr('disabled')
			}

			var _id = $(el).val()
			$.getJSON('<?=base_url("Approval_ga_supervisor/json_vehicle/")?>' + _id, function(data) {
				$('#drop_plat').html('<option value="" selected disabled>-- pilih mobil --</option>');
				$.each(data, function(k,v) {
					var status = (v.available == 1) ? '(Occupied)' : '(Available)';
					var is_disabled = (v.available == 1) ? 'disabled' : '';
					$('#drop_plat').append(`<option value="${v.id_vehicle}" ${is_disabled}>${v.nama_jenis} | ${v.merk_mobil} | ${v.plat_nomor}  ${status}</option>`);
				})
			})
		}
