...
class Approval extends CI_controller
{
...

    {
        ...
        $data['waiting'] = $this->db
            ...
            $data['wait'] = $this->db
                ...
                ->order_by('id_operational', 'desc')
                ->like('tbl_user.supervisor_name', $this->session->userdata('username'))
                ->get_where('tbl_operational', ['waiting_check_out' => 'wait', 'time_waiting_cek_out' => null,  'MONTH(tbl_operational.date_request)' => $mount, 'time_out !=' => null])
                ->result();
    ...
    function update($id, $approval_status)
    {
        ...
        $this->db->update('tbl_operational', ['action_by' => $username, 'approval_sp_status' => $approval_sp_status, 'approval_sp_at' => $approval_sp_at,'waiting_check_out' => $waiting_check_out, 'time_waiting_cek_out' => $time_waiting_cek_out], ['id_operational' => $id]);
    ...
    {
        $approval_status = $approval_status == 'approve' ? 'approved' : 'rejected';
        $this->db->update('tbl_operational', ['approval_sp' => date('Y-m-d H:i:s')], ['id_operational' => $id]);
